...
In this tutorial we will create a speechd module that generates output using Kokoro. It will be able to change voices and change the speed of the tts.
...
pip install py-speech-module "kokoro>=0.9.4" soundfile
```
...
import logging
import torch
from speechd_module import SpeechServer, SpeechDispatch, parse_ssml
import io
...
This should like somewhat similar to the previous example. But lets go through the methods and explain them.
...
This is a callback that will get called when we are asked to change the voice being used. Here we also are updating the pipeline since the `lang_code` is being set in the pipeline and the voice and language are linked together.
...
We showed this callback in the previous tutorial. It is of course a callback which is called when we are asked to speak something. 
...
The first part of this we are creating a in memory file, using pythons built-in `BytesIO`. We then can use sound file to write our audio to that BytesIO object. We use `subtype` to specify what the input audio format was. Next we need to reset the position on our in memory file, we do this with a seek. Then finally we are able to read the file back out in a format we can use.

```python
yield {"audio": audio_data, "rate": output_sample_rate, "mark": mark}
```

This last line is used to 'return' the output of a chunk of audio along with the sample rate and mark. 
